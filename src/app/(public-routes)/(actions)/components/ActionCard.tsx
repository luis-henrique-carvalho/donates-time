...
const ActionCard = ({ action, variant = 'default' }: Props) => {
    const { title, status, ong, start_date, end_date, max_volunteers, volunteer_count, category, description } = action
    ...
    const volunteerPercentage = max_volunteers > 0 ? (volunteer_count / max_volunteers) * 100 : 0;
                            ...
                            {volunteers.map((volunteer, index) => {
                                return (
                                    <Avatar key={index}>
                                        ...
                                        <AvatarFallback>{volunteer.user_name.charAt(0)}</AvatarFallback>
                                    </Avatar>
                                )
                            })}
                        </div>
